...
//       CheckLabeledStmtSeq(result.body) ;
     ...
     { AST.Process result = new AST.Process() ;
       ...
       if (PcalParams.distpcalFlag)  {
         if (   PeekAtAlgToken(1).equals("begin")
                || PeekAtAlgToken(1).equals("{")
                || PeekAtAlgToken(1).equals("replicate")
                // when using p-syntax
         ...
         if (PeekAtAlgToken(1).equals("{") || PeekAtAlgToken(1).equals("begin")) { 
           while (PeekAtAlgToken(1).equals("{") || PeekAtAlgToken(1).equals("begin")) { 
            GobbleBeginOrLeftBrace();
            if(PeekAtAlgToken(1).equals("replicate")){
              System.out.println("Called replicate subprocess");    
              ...
              //read the sub-process delimiter
              //ZZ Code newsyntax
              GobbleThis("replicate");
              PCalLocation replicLoc = GetLastLocationStart() ;
              thread.col  = lastTokCol ;
              thread.line = lastTokLine ;
              if (cSyntax) { GobbleThis("(") ; } ;
              thread.name = GetAlgToken() ;
              GobbleEqualOrIf() ;
              thread.id   = result.id;
              thread.replicate = GetExpr(); 
              if (cSyntax) { GobbleThis(")") ; } ;
         ...
         /* else if (PeekAtAlgToken(1).equals("replicate")) { 
           System.out.println("Called replicate subprocess");    
           while (PeekAtAlgToken(1).equals("replicate")) { 
            thread = new AST.Thread();
            thread.index = i++;
        //    thread.id = result.id;
        //    thread.replicate = GetExpr();
        ...
        }//end begin if statement
            GobbleEndOrRightBrace("replicate");
            if(pSyntax) {
              GobbleThis(";");
            }
     ...
     { String tok = PeekAtAlgToken(1) ;
       ...
       ********************************************************************/
       while (! (   PeekAtAlgToken(1).equals("begin")
                 ...
                 //ZZ Code
                 || PeekAtAlgToken(1).equals("replicate")
                // || PeekAtAlgToken(1).equals("subsubprocess")
                 || PeekAtAlgToken(1).equals("subprocess")) )
       ...
       if ( str.equals(";") )
         { String nxt = PeekAtAlgToken(1) ;
           if (    nxt.equals("end")
                ...
                //ZZ Code
                || nxt.equals("subsubprocess")
                || nxt.equals("replicate")
...
//      }
  ...
  public static Vector GetChannelDecls(String decl, String type) throws ParseAlgorithmException {
		...
		while (!(PeekAtAlgToken(1).equals("begin") || PeekAtAlgToken(1).equals("{")
				...
				|| PeekAtAlgToken(1).equals("subprocess")
        || PeekAtAlgToken(1).equals("replicate")  //ZZ Code
        || PeekAtAlgToken(1).equals("subsubprocess")  //ZZ Code
       )) {
