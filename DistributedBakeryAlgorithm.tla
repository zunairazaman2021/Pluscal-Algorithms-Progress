...
fair process (bakery \in Processes)
    \*Main Thread
    variables  procs =Processes \ {self}, procs3 =Processes \ {self};
    {L1: while (TRUE){
          choosing[self] := TRUE;
    M:  numbers[self] := 1 + max(numbers);
          choosing[self] := FALSE;
    PreWait: await pc[self][2] = "PreL2" /\ pc[self][3] = "PreL3";
    wait: await procs3 = {};
    CS:   skip;
          numbers[self] :=  0;
           };\*end while
...
***)
\* BEGIN TRANSLATION - the hash of the PCal code: PCal-d097ca51a0aa77e69abcf8c80418df50
...
PreWait(self) == /\ pc[self][1]  = "PreWait"
                 /\ pc[self][2] = "PreL2" /\ pc[self][3] = "PreL3"
...
L2(self) == /\ pc[self][2]  = "L2"
            /\ IF procs[self] # {}
                  THEN /\ \E i \in procs[self]:
                       ...
                       /\ pc' = [pc EXCEPT ![self][2] = "L2"]
                  ELSE /\ pc' = [pc EXCEPT ![self][2] = "POSTL2"]
                       /\ procs' = procs
...
L3(self) == /\ pc[self][3]  = "L3"
            /\ IF procs3[self] # {}
                  THEN /\ \E i \in procs3[self] \ procs[self]:
                       ...
                       /\ pc' = [pc EXCEPT ![self][3] = "L3"]
                  ELSE /\ pc' = [pc EXCEPT ![self][3] = "POSTL3"]
                       /\ UNCHANGED procs3
...
\* END TRANSLATION - the hash of the generated TLA code (remove to silence divergence warnings): TLA-da07ec362c4c2aa2bc950c8f2576df2b
...
\* Last modified Mon Apr 04 13:10:11 GMT 2022 by zunai
